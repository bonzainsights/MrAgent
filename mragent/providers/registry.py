...
class ProviderSpec:
    ...
    display_name: str = ""  # shown in `mragent status`
...
PROVIDERS: tuple[ProviderSpec, ...] = (
    ...
    # === MRAgent additions ==================================================
    # NVIDIA NIM: OpenAI-compatible gateway; free credits on registration at
    # build.nvidia.com. Keys start with "nvapi-".
    ProviderSpec(
        name="nvidia_nim",
        keywords=("nvidia", "nim", "nemotron", "nvapi"),
        env_key="NVIDIA_API_KEY",
        display_name="NVIDIA NIM",
        litellm_prefix="openai",     # model → openai/model (OpenAI-compatible)
    ...
    # Qwen OAuth: free browser-based login, ~2000 req/day, no API key needed.
    # is_direct=True so it bypasses LiteLLM; is_oauth=True to skip key check.
    ProviderSpec(
        name="qwen_oauth",
        keywords=("qwen_oauth",),
        env_key="",            # OAuth-based, no API key
        display_name="Qwen (OAuth — free)",
        litellm_prefix="",
        skip_prefixes=(),
        env_extras=(),
        is_gateway=False,
        ...
        detect_by_key_prefix="",
        detect_by_base_keyword="",
        default_api_base="https://chat.qwen.ai/api",
        strip_model_prefix=False,
        model_overrides=(),
        is_oauth=True,
        is_direct=True,
