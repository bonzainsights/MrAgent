...
import mragent.channels.matrix as matrix_module
from mragent.bus.events import OutboundMessage
from mragent.bus.queue import MessageBus
from mragent.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from mragent.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("mragent.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "mragent.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("mragent.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "mragent.channels.matrix.asyncio.create_task", _fake_create_task
    )
