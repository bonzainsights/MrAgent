...
def get_image():
    ...
    Default: NVIDIA FLUX. Override with IMAGE_PROVIDER=google env var.
    """
    global _image_provider
    if _image_provider is None:
        import os
        provider_choice = os.getenv("IMAGE_PROVIDER", "flux").lower()
        if provider_choice == "google" and os.getenv("GOOGLE_AI_STUDIO_KEY"):
            from providers.google_image import GoogleImageProvider
            _image_provider = GoogleImageProvider()
            logger.info("Image provider: Google AI Studio")
        else:
            from providers.nvidia_image import NvidiaImageProvider
            _image_provider = NvidiaImageProvider()
            logger.info("Image provider: NVIDIA FLUX (default)")
